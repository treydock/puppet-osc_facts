...
describe 'osc_gpu_type Fact' do
  context 'example' do
    ...
    it 'should be nil for c6320 nodes' do
      ...
      expect(Facter.fact(:osc_gpu_type).value).to be_nil
    end

    it 'should be p100 for r730 nodes' do
      allow(Facter).to receive(:value).with(:hostname).and_return('compute02')
      expect(Facter.fact(:osc_gpu_type).value).to eq('p100')
    end
